...
We also have another tutorial for [running OPAL with an example docker-compose configuration](get_started_with_opal_docker_compose_tutorial). This other tutorial is better for **learning** about OPAL in a **live playground environment**.
...
<table>
  <tbody>
    ...
    <tr>
      <td valign="top" align="left">Use the <a href="get_started_with_opal_docker_compose_tutorial">other</a> tutorial if you</td>
      <td valign="top" align="left">
        <ul>
...
Our recommendation is to start with the [docker-compose playground](get_started_with_opal_docker_compose_tutorial) (quicker setup, better as a first tutorial) and then come back here and learn how to set up OPAL with a real configuration.
...
For more info, check out this tutorial: [How to track a git repo](/howto/track_a_git_repo).

### Step 5: Server config - data sources
The OPAL server serves as the **base data source configuration** for the OPAL client. 
...
The data sources configured on the server will be fetched **by the client** every time it decides it needs to fetch the **entire** data configuration (i.e., when the client first loads, after a period of disconnection from the server, etc.). 
This configuration must always point to a complete and up-to-date representation of the data (not a "delta").
...
* **Where to place the data in the OPA cache:** although not specified, this entry uses the default of `/` which means at the root of the OPA document hierarchy. You can specify another path with `dst_path` (check the [schema](https://github.com/permitio/opal/blob/master/packages/opal-common/opal_common/schemas/data.py#L8)).
...
Env Var Name | Function
:--- | :---
OPAL_INLINE_OPA_CONFIG | The value of this var should be an [OpaServerOptions](https://github.com/permitio/opal/blob/master/packages/opal-client/opal_client/opa/options.py#L19) pydantic model encoded into JSON string. 
The process is similar to the one we showed on how to encode the value of [OPAL_DATA_CONFIG_SOURCES](#encode-data-sources).
...
[We have a separate tutorial on how to trigger updates](../howto/trigger_data_updates).
