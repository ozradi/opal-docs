---
sidebar_position: 2
slug: get-started-opal-docker-containers
title: Get started with OPAL docker containers
---
...
We also have another tutorial for [running OPAL with an example docker-compose configuration](https://github.com/permitio/opal/blob/master/docs/howto/get_started_with_opal_docker_compose_tutorial.md). This other tutorial is better for **learning** about OPAL in a **live playground environment**.
...
<table>
  <tbody>
    ...
    <tr>
      <td valign="top" align="left">Use the <a href="https://github.com/permitio/opal/blob/master/docs/howto/get_started_with_opal_docker_compose_tutorial.md">other</a> tutorial if you</td>
      <td valign="top" align="left">
        <ul>
          ...
          <li>Want to <strong>learn</strong> about OPAL's core features and see what OPAL can do for you.</li>
        </ul>
      </td>
    </tr>
  </tbody>
...
Our recommendation is to start with the [docker-compose playground](https://github.com/permitio/opal/blob/master/docs/howto/get_started_with_opal_docker_compose_tutorial.md) (quicker setup, better as a first tutorial) and then come back here and learn how to set up OPAL with a real configuration.
  ...
  <tbody>
    ...
    <tr>
      ...
      <ul>
        ...
        <li>Keeps the OPA agent cache up to date with real-time updates pushed from the server</li>
        <li>Can selectively subscribe to specific topics of policy code (rego) and policy data</li>
        <li>Fetches data from multiple sources (i.e., DBs, APIs, 3rd party services)</li>
      </ul>
      </td>
    </tr>
    <tr>
      ...
      <ul>
        ...
        <li>Intended to be used when you prefer to deploy OPA separately in its container.</li>
      </ul>
      </td>
    </tr>
  </tbody>
...
Mounts the `~/.ssh` dir as volume | Varies between environments, i.e., in AWS ECS, you would mount volumes via the task definition.
Passes the following env vars to the docker container as config: `OPAL_AUTH_PRIVATE_KEY`, `OPAL_AUTH_PUBLIC_KEY`, `OPAL_POLICY_REPO_URL`. | Varies between environments, i.e., in AWS ECS, you would specify env vars values via the task definition.
Exposes port 7002 on the host machine. | Varies between environments, i.e., in AWS ECS, you would specify exposed ports in the task definition and will have to expose these ports via a load balancer.

### <a name="configuration"></a> Configuration Variables
We will now explain how to pass configuration variables to OPAL.

* In its dockerized form, OPAL server and client containers pick up their configuration variables from **environment variables** prefixed with `OPAL_` (i.e., `OPAL_DATA_CONFIG_SOURCES`, `OPAL_POLICY_REPO_URL`, etc.).
...
You should read and understand [OPAL Security Model](https://github.com/permitio/opal/blob/master/docs/security.md) before going to production.

However, we will list the mandatory checklist briefly here as well:
...
* Sensitive configuration variables (i.e., environment variables with sensitive values) should **always** be stored in a dedicated **Secret Store**.
  ...
  * **NEVER EVER EVER** store secrets as part of your source code (i.e., in your git repository).
...
```
If you run in a cloud environment (i.e., AWS ECS), specify `permitio/opal-server` in your task definition or equivalent.

Running the opal server container is simply a command of [docker run](#example-docker-run), but we need to pipe to the OPAL server container the necessary configuhttration it needs via **environment variables**. The following sections will explain each class of configuration variables and how to set their values, after which we will demonstrate real examples.

### Step 2: Server config - broadcast interface

#### 1) Deploying the broadcast channel backbone service (optional)

When scaling the OPAL Server to **multiple workers** and/or **multiple containers**, we use a **broadcast channel** to sync between all the instances of the OPAL Server. In order words, communication on the broadcast channel is **communication between OPAL servers** and is not related to the OPAL client.
...
Deploying the actual service used for broadcast (i.e., Redis) is outside the scope of this tutorial. The easiest way is to use a managed service (i.e., AWS RDS, AWS ElastiCache, etc.), but you can also deploy your own dockers.
...
OPAL server is responsible for tracking policy changes and pushing them to OPAL clients.

At the moment, OPAL tracks a git repository as the **policy source**.
...
<table>
  <tbody>
    ...
    <tr>
      ...
      <td>
        <ul>
          <li>The repo URL that the policy repo is located at.</li>
...
If your tracked policy repo is <strong>private</strong>, you should declare this env var to authenticate and successfully clone the repo:
    ...
    <tr>
      ...
      <td>
        <ul>
          ...
          <li>
            The passed value must be the contents of the SSH key in one line (replace new-line with an underscore, i.e., <code>\n</code> with <code>_</code>)
          </li>
        </ul>
      </td>
    </tr>
...
<table>
  <tbody>
    ...
    <tr>
      ...
      <td>Name of the git branch to track policy files (default: `master`)</td>
    </tr>
  </tbody>
</table>
...
* If new commits are pushed to the tracked git repository **that affect rego or data files**, the updated policy will be pushed to OPA automatically in real-time by OPAL. -->

Currently, the OPAL server supports two ways to detect changes in the policy git repo:
* **Polling in fixed intervals** - checks every X seconds if new commits are available.
* **Github Webhooks** - if the git repo is stored on Github - you may set up a webhook (we plan to expand to a generic webhook soon).
...
It is much more recommended to use webhooks if your policy repo is stored in a supported service (currently Github, we are working on expanding this). Webhooks are much more efficient with network traffic and won't contaminate your logs.

If your server is hosted at `https://opal.yourdomain.com`, the webhook URL you must set up with your webhook provider (i.e., Github) is `https://opal.yourdomain.com/webhook`. See [GitHub's guide on configuring webhooks](https://docs.github.com/en/developers/webhooks-and-events/creating-webhooks).
...
The OPAL server serves as the **base data source configuration** for the OPAL client. The configuration is structured as **directives** for the client; each directive specifies **what to fetch** (URL) and **where to put it** in OPA data document hierarchy (destination path).

The data sources configured on the server will be fetched **by the client** every time it decides it needs to fetch the **entire** data configuration (i.e., when the client first loads, after a period of disconnection from the server, etc.). This configuration must always point to a complete and up-to-date representation of the data (not a "delta").
...
Each object in `entries` (schema: [DataSourceEntry](https://github.com/permitio/opal/blob/master/opal_common/schemas/data.py#L8)) is a **directive** that tells the OPAL client to fetch the data and place it in the OPA cache using the [Data API](https://www.openpolicyagent.org/docs/latest/rest-api/#data-api).
* **From where to fetch:** we tell the OPAL client to fetch data from the [Permit.io API](https://api.permit.io/redoc) (specifically, from the `policy-config` endpoint).
* **How to fetch (optional):** we can direct the client to use a specific configuration when fetching the data. For example, here we tell the client to use a specific HTTP Authorization header with a bearer token to authenticate to the API.
* **Where to place the data in the OPA cache:** although not specified, this entry uses the default of `/` which means at the root of the OPA document hierarchy. You can specify another path with `dst_path` (check the [schema](https://github.com/permitio/opal/blob/master/opal_common/schemas/data.py#L8)).
...
Declaring these parameters and passing them to the OPAL server will cause the server to run in **secure mode**, which means client identity verification will be active. 
All the values in this section are sensitive, and you should place them in a [secrets store](#security-considerations) in production.
    ...
    <tr>
      ...
      <td>
        <ul>
          ...
          <li>
            Usually, public keys already fit into one line. If not, encoding is the same as for the private key (replace new-line with an underscore, i.e., <code>\n</code> with <code>_</code>).
          </li>
        </ul>
      </td>
    </tr>
...
You can choose any secret you'd like, but as we've shown you [before](#generate-secret), the OPAL CLI can be used to generate cryptographically strong secrets easily.
...
To summarize, the previous steps guided you on how to pick the values of the configuration variables needed to run the OPAL server.
...
* Declare your container configuration in code, i.e., AWS ECS task definition file, Helm chart, etc.
...
```
If you run in a cloud environment (i.e., AWS ECS), specify `permitio/opal-client` in your task definition or equivalent.
...
In production environments, the OPAL server **should** be running in **secure mode**, and the OPAL client must have a valid identity token (which is a signed JWT) to successfully connect to the server.

Obtaining a token is easy. You'll need the OPAL server's **master token** to request a JWT token.
...
* You deployed the OPAL server to `https://opal.yourdomain.com`
* The master token of your deployment is `MY_MASTER_TOKEN`.
  * However, if you followed our tutorial for the server, you probably generated one [here](#generate-secret), which is the master token you should use.
...
For example, if the OPAL server is available at `https://opal.yourdomain.com`:
```sh
...
If you do not specify data topics in your configuration, the OPAL client will automatically subscribe to a single topic: `policy_data` (the default).
...
To override the default configuration, you'll need to set this env var:
Env Var Name | Function
:--- | :---
...
If OPA is deployed separately from OPAL (i.e., using the standalone image), you should define the URI of the OPA instance you want to manage with the OPAL client with this env var:

Env Var Name | Function
:--- | :---
OPAL_POLICY_STORE_URL | The internet address (URI) of the deployed standalone OPA.

For example, if the standalone OPA is available at `https://opa.billing.yourdomain.com:8181`:
```sh
...
#### 1) Get the client's image
First, download the OPAL client docker image:
```sh
docker pull permitio/opal-client
```
...
```sh
# let's say this is the (shortened) token we obtained from the OPAL server
export OPAL_CLIENT_TOKEN=eyJ0...8wsk
# and this is where we deployed the OPAL server
export OPAL_SERVER_URL=https://opal.yourdomain.com
...
Now that OPAL is live, we can use the OPAL server to push updates to the OPAL clients in real time.

[We have a separate tutorial on how to trigger updates](https://github.com/permitio/opal/blob/master/docs/howto/trigger_data_updates.md).
...
OPAL client is configured to keep the connection with server open, and in case of disconnection - retry again and again to reconnect. But if something is wrong with the server address (server is down, unreachable, or DNS not configured), you might see the following logs:
```
...
Env Var Name | Function
:--- | :---
OPAL_LOG_MODULE_INCLUDE_LIST | A directive to include logs outputted by the modules appearing in the lists. i.e., setting `OPAL_LOG_MODULE_INCLUDE_LIST=["uvicorn.protocols.http"]` means that all log lines by the module `uvicorn.protocols.http` will be included in the log.
