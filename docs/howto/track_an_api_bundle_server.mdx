---
sidebar_position: 6
title: Track policy changes from an API 
---
...
Since the example `docker-compose-api-policy-source-example.yml` makes OPAL track the API bundle server (which serves the files from `/docker/docker_files/bundle files`). In order to see how a bundle update can affect the policy in real-time, we can run the following commands to trigger a policy update:

```bash
cd docker/docker_files/bundle_files
mv bundle.tar.gz{,.bak1}; mv bundle.tar.gz{.bak,}; mv bundle.tar.gz.bak{1,} 
# this command swaps the two bundle files you have, to trigger a policy change
```
